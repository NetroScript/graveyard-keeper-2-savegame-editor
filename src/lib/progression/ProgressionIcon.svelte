<script lang="ts">
  ...
  let { name, kind = "sprite", label = "", recolor = true, size }: { name?: string | null; kind?: "sprite" | "font"; label?: string; recolor?: boolean; size?: string } = $props();
...
<span class="progression-icon" style:--icon-size={size}>{#if url}<img src={url} alt={label} />{:else}<span aria-hidden="true">?</span>{/if}</span>
<style>
